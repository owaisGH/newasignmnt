...
public class PhotoController extends BaseController {
    ...
    private static final String DEFAULT_PHOTO = "fileneta-photo.png";
    
    @Value("${filemetas.photo.storage}")
    private String photoDir;
    
    @Value("${filemetas.photo.resize-width}")
    private int photoWidth;
    
    @Value("${filemetas.photo.resize-height}")
    private int photoHeight;
    
    @Autowired
    private MultipartResolver multipartResolver;
    
    @RequestMapping(value="/{filemetaId}", method = GET)
    public HttpEntity<byte[]> getPhoto(@PathVariable String filemetaId, WebRequest request) throws IOException {
        File photoFile = new File(photoDir, filemetaId + "." + EXT_NAME);
        if (!photoFile.exists()) {
    ...
    @RequestMapping(value="/{filemetaId}", method = POST)
    public void uploadPhoto(@PathVariable String filemetaId, HttpServletRequest request) throws IOException {
        MultipartHttpServletRequest multipartRequest = multipartResolver.resolveMultipart(request);
        
        MultipartFile file = multipartRequest.getFile("file");
        File uploadFile = File.createTempFile("filemeta-", filemetaId);
